...
def start(update: Update, context: CallbackContext):
	update.message.reply_text("Hi! I'm a bot that can upload video files. Just send me a video file and I'll do the rest.")
...
def handle_directory_upload(update: Update, context: CallbackContext):
	...
	sorted_episode_list = sorted(video_files, key=lambda file: extract_episode_number(file))
	...
	for video_file in sorted_episode_list:
		...
		with open(video_file, "rb") as f:
			try:    
				update.message.reply_document(document=f, caption=video_file.replace("/home/avik/Downloads/torrents/FamilyGuyFull/", ''))
				#sleep(1)
			except:
				try:
					update.message.reply_document(document=f, caption=video_file.replace("/home/avik/Downloads/torrents/FamilyGuyFull/", ''))
...
def main():
	...
	updater.dispatcher.add_handler(CommandHandler("dir", handle_directory_upload, pass_args=True))
